...
class TelegramBot:
    ...
    async def _emoji_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            ...
            text += (
                ...
                f"*Примеры\\:*\n"
                f"`/emoji 🔥` - закрепить огонь\n"
                f"`/emoji ✨` - закрепить искры\n\n"
                f"*Посмотреть доступные\\:*\n"
                f"`/availableemojis`"
            )
        ...
        if reserved_owner and reserved_owner != user.id:
            if is_admin(user.id):
                ...
                await update.message.reply_text(
                    ...
                    f"Используйте `/availableemojis` чтобы увидеть свободные\\.",
                    parse_mode=ParseMode.MARKDOWN_V2
                )
            return
    ...
    async def _myreservations_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if reserved_emoji:
            ...
            text += f"1\\. Используйте `/availableemojis`\n"
            text += f"2\\. Выберите понравившийся эмодзи\n"
            text += f"3\\. Используйте `/emoji [эмодзи]`\n\n"
            text += f"*Текущий эмодзи\\:* {current_emoji}\n"
    ...
    async def _edit_message_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                ...
                "*Использование\\:*\n"
                "`/edit ID\\_сообщения` - начать редактирование\n\n"
                "*Для редактирования\\:*\n"
    ...
    async def _encrypted_admin_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            ...
            available_cmds = "\n".join([f"• `{cmd}`" for cmd in self.encrypted_commands.keys()][:10])
            
            # Используем HTML разметку вместо MarkdownV2
            text = (
                f"🔐 <b>Шифрованные админ команды</b>\n\n"
                f"<b>Использование:</b>\n"
            ...
            # Используем HTML вместо MARKDOWN_V2
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
            return
    ...
    async def _encrypted_admin_session(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            f"🔐 <b>Админ сессия создана</b>\n\n"
            f"<b>Токен:</b> <code>{session_token[:16]}...</code>\n"
            f"<b>Действует до:</b> {self.admin_sessions[user.id]['expires'].strftime('%d.%m.%Y %H:%M')}\n"
            f"<b>Команд использовано:</b> 0\n\n"
            f"<b>Шифрованная сессия:</b>\n"
    ...
    async def _admin_users_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            ...
            f"*Шифрованные команды\\:*\n"
            f"`/as22f2ffj8 e5f6g7h8` - список пользователей\n"
            f"`/as22f2ffj8 m3n4o5p6 [ID]` - забанить\n"
            f"`/as22f2ffj8 q7r8s9t0 [ID]` - разбанить\n"
            f"`/as22f2ffj8 u1v2w3x4 [ID] [дни]` - выдать премиум\n\n"
            f"*Статистика\\:*\n"
    ...
    async def _admin_emoji_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            f"🎨 *Управление эмодзи*\n\n"
            f"*Шифрованные команды\\:*\n"
            f"`/as22f2ffj8 y5z6a7b8` - список эмодзи\n"
            f"`/as22f2ffj8 c9d0e1f2 [эмодзи]` - освободить эмодзи\n\n"
            f"*Статистика\\:*\n"
    ...
    async def _admin_settings_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            ...
            f"*Шифрованные команды\\:*\n"
            f"`/as22f2ffj8 g3h4i5j6 [текст]` - рассылка\n"
            f"`/as22f2ffj8 k7l8m9n0` - сброс БД\n"
            f"`/as22f2ffj8 s5t6u7v8` - отладка\n"
            f"`/as22f2ffj8 w9x0y1z2` - логи\n\n"
            f"*Системные команды\\:*\n"
            f"`/b3g5h7j9k1` - создать сессию\n"
            f"`/c4d6f8h0j2` - статус системы"
        )
